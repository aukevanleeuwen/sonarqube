name 'sonarqube'
description 'Installs/Configures SonarQube Server and SonarQube Scanner'
long_description 'Installs/Configures SonarQube Server and SonarQube SCanner'
version '0.3.3'

maintainer 'Christian E Willman'
maintainer_email 'christian@willman.io'

license 'MIT'

supports 'centos'
supports 'ubuntu'

issues_url 'https://github.com/aukevanleeuwen/sonarqube/issues' if respond_to?(:issues_url)
source_url 'https://github.com/aukevanleeuwen/sonarqube.git' if respond_to?(:source_url)
